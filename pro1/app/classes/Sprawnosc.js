...
export default function Sprawnosc({sprawnosc}){
    const {user,deleteFromList,zmienEdycje}=useContext(GlobalContext)

    if(!sprawnosc)return<p>nie ma sprawnosci</p>
    return(
        <div>
            {user.typUzytkownika==="Druzynowy"&& <FaTrash onClick={()=>deleteFromList(sprawnosc.id,"Sprawnosc")}/>}
            ...
            {(user.typUzytkownika==="Przyboczny"||user.typUzytkownika==="Druzynowy")&& <div>
                <p>{sprawnosc.nazwa}</p>
