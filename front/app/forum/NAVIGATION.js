...
export default function Navigation() {
    const { pushClick,loading } = useContext(GlobalContext);
    ...
    const buttons = [
        ...
        return (
            <div>
                ...
                <div
                    className="md:hidden fixed top-[50px] left-0 w-full bg-[#3A4F39] z-10 px-4 py-2 flex justify-between items-center">
                ...
                {/*// Widok desktopowy*/}
                <div
                    className="hidden  md:flex  md:flex-col md:space-x-2 fixed top-0 left-0 w-[250px] bg-[#3A4F39] h-full p-2 justify-center">
                    {buttons.map((btn, idx) => (
                        <button
                            key={idx}
                            onClick={(e) => pushClick(e, btn.path)}
                            className="mb-2 bg-[#405E3F] text-white px-3 py-2 rounded shadow w-full"
                        >
                            {btn.label}
                        </button>
                    ))}
